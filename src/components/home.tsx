          ...
          <tbody className="bg-black divide-y divide-gray-700">
            {paginatedBids.map((bid, index) => (
              <tr key={bid.encodedOrderId}>
                <td
                  className="px-6 py-4 whitespace-nowrap flex flex-row gap-2 items-center"
                  style={
                ...
                >
                  ...
                  {bid.profileName === null
                    ? bid.user.address.slice(0, 5) +
                      "..." +
                      bid.user.address.slice(
                        bid.user.address.length - 4,
                        bid.user.address.length
                      )
                    : bid.profileName}
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap flex flex-row gap-2 items-center">
                  ...
                  <div>
                    {bid.tokenProfileImage ? (
                      <img
                        className="w-8 h-8 rounded-full"
                        src={bid.tokenProfileImage ?? ""}
                        alt="token profile Image"
                      />
                    ) : (
                      <div className="bg-gradient-to-b from-violet-500 to-blue-600 w-8 h-8 rounded-full shadow-lg"></div>
                    )}
                  </div>
                  {bid.tokenProfileName ??
                    bid.auctioningToken?.slice(0, 5) +
                      "..." +
                      bid.auctioningToken?.slice(
                        bid.auctioningToken?.length - 4,
                        bid.auctioningToken?.length
                      )}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
